...
class Cell {
	constructor(type, relations, selectable) {
		this.type = type;
...
const BACTS_TYPES = CELL_TYPES.length;
...
function init() {
	...
	for (let i = 0; i < DIMMENSION; i++) {
		...
		for (let j = 0; j < DIMMENSION; j++) {
			const newCell = new Cell(matrix[i][j].type, matrix[i][j].relations, matrix[i][j].selectable);
			MATRIX[i][j] = newCell;
			if (newCell.type === 'empty') {
				const newBactId = getRandomInt(0, BACTS_TYPES);
...
function fillEmptyCells(way) {
	moves = [];
	for (var i = DIMMENSION - 2; i > -1; i--) { //beginning from 2 row, cuz bact cant drop from first row
...
function spawn() {
	...
	const currentSpawner = spawners.shift();
...
function dropBact(row, col) {
	if (!MATRIX[row + 1]) { //bottom row
		return;
	}
	// console.log(MATRIX[row][col]);
	if (MATRIX[row][col].relations.blocked.length > 0) { //bottom row
		return;
	}
...
function releaseChain() {
	...
	chain.bacts.forEach(cellId => {
		...
		const currentCell = MATRIX[corrdinates.row][corrdinates.col];
...
function showBlockers(cell) {
	if (cell.relations.blocked && cell.relations.blocked.length > 0) {
		cell.relations.blocked.forEach(blocker => {
			const cellId = `${blocker.row}_${blocker.col}`;
...
function releaseRelations(cell, cellCoordinates) {
	...
	let blockerKeys = []; //indexes of released relations for BLOCKER cell

	//iterate through BLOCKER relations
	_.forEach(cell.relations.blocks, (relation, blockerKey) => {
		...
		const relationString = `${relation.row}_${relation.col}`;
